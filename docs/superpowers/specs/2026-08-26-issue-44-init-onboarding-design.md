...
Der normale Einstieg wird `npm i @tomtastisch/agent-governance` gefolgt von `npx agent-governance init`. `init` ist eine interaktive Orchestrierung über der vorhandenen `InstallerTransaction`; die einzige mutierende Engine bleibt `plan -> install -> verify`. Die acht bisherigen Commands bleiben rückwärtskompatibel und verlangen weiterhin ihre expliziten Pfadargumente. Management-Namespaces aus #42 und die Startup-/Readiness-Zustandsmaschine aus #37 bleiben außerhalb des Scopes.
