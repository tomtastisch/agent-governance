...
keine Harness-Erkennung, Adapter, Hooks oder weitere Runtime-Verhalten hinzu. Der normale
öffentliche Einstieg ist stets `npm i @tomtastisch/agent-governance` und danach
`npx agent-governance init`. Die vollständige Semantik von Commands und Optionen steht in der
[Installer-CLI-Referenz](installer-cli-reference.md).

Die folgenden Advanced-/Automation-Rezepte verwenden `--scope global`, einen expliziten absoluten `--installation-root`, einen
