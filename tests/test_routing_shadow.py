...
def _git_baseline_resolvable() -> bool:
    try:
        subprocess.run(
            ["git", "cat-file", "-e", f"{BASELINE['head']}^{{commit}}"],
...
@unittest.skipUnless(
    _git_baseline_resolvable(),
    "Baseline-Commit nicht in der Git-Historie (shallow clone)",
)
class ShadowBaselineAncestryContract(unittest.TestCase):
