...
import { access, lstat, mkdir, open, readFile, rename, symlink, writeFile } from "node:fs/promises";
...
import { probeNativeFilesystemCapability, secureCreateDirectory, secureCreateNoReplace, secureRemoveFile, secureRenameDirectoryNoReplace, secureRenameNoReplace, secureWriteFile, type FileSnapshotIdentity } from "../../src/native-filesystem.ts";
...
async function captureFileSnapshot(path: string): Promise<FileSnapshotIdentity> {
  const stat = await lstat(path, { bigint: true });
  return { device: stat.dev, inode: stat.ino, mode: Number(stat.mode), size: stat.size, mtimeNs: stat.mtimeNs, ctimeNs: stat.ctimeNs };
}
...
test("native create removes its exclusive target after an injected deferred close failure", async () => {
  const root = await createTestRoot("agent-governance-native-create-close-"); const directory = join(root, "directory"); await mkdir(directory); const repository = join(dirname(fileURLToPath(import.meta.url)), "..", ".."); const output = join(root, "close-failure.node");
  const includeCandidates = [join(dirname(process.execPath), "..", "include", "node"), "/usr/local/include/node", "/opt/homebrew/include/node", "/usr/include/node"]; let include: string | undefined; for (const candidate of includeCandidates) { try { await access(join(candidate, "node_api.h")); include = candidate; break; } catch { /* next local header root */ } } assert.notEqual(include, undefined);
  const platformFlags = process.platform === "darwin" ? ["-bundle", "-undefined", "dynamic_lookup"] : ["-shared", "-fPIC"]; const build = spawnSync(process.env.CC ?? "cc", ["-std=c11", "-O2", "-Wall", "-Wextra", "-Werror", ...platformFlags, `-I${include!}`, "-DNODE_GYP_MODULE_NAME=agent_governance_fs", "-DAGENT_GOVERNANCE_TEST_FAIL_CREATE_CLOSE=1", join(repository, "native", "agent_governance_fs.c"), "-o", output]); assert.equal(build.status, 0, build.stderr.toString());
  const binding = createRequire(import.meta.url)(output) as NativeTestBinding; const handle = await open(directory, "r"); try { const identity = await handle.stat({ bigint: true }); assert.throws(() => binding.secureCreateNoReplace(handle.fd, "entry.md", identity.dev, identity.ino, Buffer.from("content\n")), /close|input\/output|I\/O/i); await assert.rejects(access(join(directory, "entry.md"))); } finally { await handle.close(); }
  await secureCreateNoReplace({ directory, name: "entry.md", directoryIdentity: await captureIdentity(directory) }, Buffer.from("retry\n")); assert.equal(await readFile(join(directory, "entry.md"), "utf8"), "retry\n");
});

test("native deferred-close cleanup preserves a same-inode foreign change", async () => {
  const root = await createTestRoot("agent-governance-native-create-close-race-"); const directory = join(root, "directory"); await mkdir(directory); const repository = join(dirname(fileURLToPath(import.meta.url)), "..", ".."); const output = join(root, "close-race.node");
  const includeCandidates = [join(dirname(process.execPath), "..", "include", "node"), "/usr/local/include/node", "/opt/homebrew/include/node", "/usr/include/node"]; let include: string | undefined; for (const candidate of includeCandidates) { try { await access(join(candidate, "node_api.h")); include = candidate; break; } catch { /* next local header root */ } } assert.notEqual(include, undefined);
  const platformFlags = process.platform === "darwin" ? ["-bundle", "-undefined", "dynamic_lookup"] : ["-shared", "-fPIC"]; const build = spawnSync(process.env.CC ?? "cc", ["-std=c11", "-O2", "-Wall", "-Wextra", "-Werror", ...platformFlags, `-I${include!}`, "-DNODE_GYP_MODULE_NAME=agent_governance_fs", "-DAGENT_GOVERNANCE_TEST_FAIL_CREATE_CLOSE=1", "-DAGENT_GOVERNANCE_TEST_MUTATE_CREATE_CLOSE=1", join(repository, "native", "agent_governance_fs.c"), "-o", output]); assert.equal(build.status, 0, build.stderr.toString());
  const binding = createRequire(import.meta.url)(output) as NativeTestBinding; const handle = await open(directory, "r"); try { const identity = await handle.stat({ bigint: true }); assert.throws(() => binding.secureCreateNoReplace(handle.fd, "entry.md", identity.dev, identity.ino, Buffer.from("installer bytes\n")), /close|input\/output|I\/O/i); } finally { await handle.close(); }
  assert.equal(await readFile(join(directory, "entry.md"), "utf8"), "foreign bytes\n");
});
...
test("native replace and remove remain bound to the validated parent across pathname swaps", async () => {
  ...
  const entryIdentity = await captureFileSnapshot(join(parent, "entry.json")); await secureWriteFile({ directory: parent, name: "entry.json", directoryIdentity: identity, objectIdentity: entryIdentity, onDirectoryBound: async () => { renameSync(parent, retired); mkdirSync(parent); writeFileSync(join(parent, "entry.json"), "foreign\n"); swapped = true; } }, Buffer.from("trusted new\n")); assert.equal(swapped, true); assert.equal(await readFile(join(retired, "entry.json"), "utf8"), "trusted new\n"); assert.equal(await readFile(join(parent, "entry.json"), "utf8"), "foreign\n");
  await secureRemoveFile({ directory: retired, name: "entry.json", directoryIdentity: identity, objectIdentity: await captureFileSnapshot(join(retired, "entry.json")) }); await assert.rejects(access(join(retired, "entry.json"))); assert.equal(await readFile(join(parent, "entry.json"), "utf8"), "foreign\n");
});
...
test("native replace and remove reject a final-component substitution before the syscall", async () => {
  ...
  await writeFile(join(parent, "entry.json"), "trusted old\n"); const entryIdentity = await captureFileSnapshot(join(parent, "entry.json")); const retiredWrite = join(parent, "trusted-write-retired.json"); await assert.rejects(secureWriteFile({ directory: parent, name: "entry.json", directoryIdentity: identity, objectIdentity: entryIdentity, onDirectoryBound: async () => { await rename(join(parent, "entry.json"), retiredWrite); await writeFile(join(parent, "entry.json"), "foreign write\n"); } }, Buffer.from("installer new\n")), /identity|stale/i); assert.equal(await readFile(join(parent, "entry.json"), "utf8"), "foreign write\n");
  await writeFile(join(parent, "owner.json"), "trusted owner\n"); const ownerIdentity = await captureFileSnapshot(join(parent, "owner.json")); const retiredRemove = join(parent, "trusted-owner-retired.json"); await assert.rejects(secureRemoveFile({ directory: parent, name: "owner.json", directoryIdentity: identity, objectIdentity: ownerIdentity, onDirectoryBound: async () => { await rename(join(parent, "owner.json"), retiredRemove); await writeFile(join(parent, "owner.json"), "foreign owner\n"); } }), /identity|stale/i); assert.equal(await readFile(join(parent, "owner.json"), "utf8"), "foreign owner\n");
});
...
test("native replace and remove reject substitution after the caller snapshot", async () => {
  ...
  await writeFile(join(parent, "current.json"), "trusted current\n"); const currentIdentity = await captureFileSnapshot(join(parent, "current.json")); await rename(join(parent, "current.json"), join(parent, "trusted-current-retired.json")); await writeFile(join(parent, "current.json"), "foreign current\n"); await assert.rejects(secureWriteFile({ directory: parent, name: "current.json", directoryIdentity: parentIdentity, objectIdentity: currentIdentity }, Buffer.from("installer current\n")), /identity|stale/i); assert.equal(await readFile(join(parent, "current.json"), "utf8"), "foreign current\n");
  await writeFile(join(parent, "owner.json"), "trusted owner\n"); const ownerIdentity = await captureFileSnapshot(join(parent, "owner.json")); await rename(join(parent, "owner.json"), join(parent, "trusted-owner-retired.json")); await writeFile(join(parent, "owner.json"), "foreign owner\n"); await assert.rejects(secureRemoveFile({ directory: parent, name: "owner.json", directoryIdentity: parentIdentity, objectIdentity: ownerIdentity }), /identity|stale/i); assert.equal(await readFile(join(parent, "owner.json"), "utf8"), "foreign owner\n");
});

test("native replace and remove reject same-inode content changes after the caller snapshot", async () => {
  const root = await createTestRoot("agent-governance-native-same-inode-"); const parent = join(root, "parent"); await mkdir(parent); const parentIdentity = await captureIdentity(parent);
  await writeFile(join(parent, "current.json"), "trusted current\n"); const currentIdentity = await captureFileSnapshot(join(parent, "current.json")); await assert.rejects(secureWriteFile({ directory: parent, name: "current.json", directoryIdentity: parentIdentity, objectIdentity: currentIdentity, onDirectoryBound: async () => { await writeFile(join(parent, "current.json"), "foreign current bytes\n"); } }, Buffer.from("installer current\n")), /identity|stale/i); assert.equal(await readFile(join(parent, "current.json"), "utf8"), "foreign current bytes\n");
  await writeFile(join(parent, "owner.json"), "trusted owner\n"); const ownerIdentity = await captureFileSnapshot(join(parent, "owner.json")); await assert.rejects(secureRemoveFile({ directory: parent, name: "owner.json", directoryIdentity: parentIdentity, objectIdentity: ownerIdentity, onDirectoryBound: async () => { await writeFile(join(parent, "owner.json"), "foreign owner bytes\n"); } }), /identity|stale/i); assert.equal(await readFile(join(parent, "owner.json"), "utf8"), "foreign owner bytes\n");
});
...
test("native replace rejects a substituted visible temporary source", async () => {
  ...
  const binding = createRequire(import.meta.url)(output) as NativeTestBinding; const handle = await open(directory, "r"); try { const parent = await handle.stat({ bigint: true }); const object = await lstat(join(directory, "entry.json"), { bigint: true }); assert.throws(() => binding.secureWriteFile(handle.fd, "entry.json", parent.dev, parent.ino, object.dev, object.ino, object.mode, object.size, object.mtimeNs, object.ctimeNs, Buffer.from("installer new\n")), /temporary.*identity|stale/i); } finally { await handle.close(); } assert.equal(await readFile(join(directory, "entry.json"), "utf8"), "trusted old\n");
});
