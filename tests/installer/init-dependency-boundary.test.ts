...
test("the 1.2.0 real init path never starts a package manager or dependency repair process", async (t) => {
  ...
  assert.equal((await readFile(join(repositoryRoot, "VERSION"), "utf8")).trim(), "1.2.0");
});
