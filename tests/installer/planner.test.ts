...
test("generic plan is deterministic and excludes every harness-specific effect", () => {
  const input = { command: "install" as const, state: "FRESH" as const, entryPath: "/target/AGENTS.md", installationRoot: "/install", version: "1.0.0-rc.1" };
...
test("generic plan fails closed for tampered and recovery-required states", () => {
  for (const state of ["TAMPERED", "RECOVERY_REQUIRED"] as const) assert.throws(() => planInstallation({ command: "install", state, entryPath: "/target/A.md", installationRoot: "/install", version: "1.0.0" }), /unsafe state/);
});
test("uninstall plan preserves releases while removing only active metadata and the managed block", () => {
  const plan = planInstallation({ command: "uninstall", state: "CURRENT", entryPath: "/target/A.md", installationRoot: "/install", version: "1.0.0" });
...
test("update plan reports the inspected local-rules operation", () => {
  const plan = planInstallation({ command: "update", state: "OUTDATED", entryPath: "/target/A.md", installationRoot: "/install", version: "1.0.0", localRulesOperation: "replace", localRulesPath: "local/custom-rules.md" });
