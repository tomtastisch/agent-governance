...
test("update dry-run reports the local-rules file carried into a new release", async () => { const f = await fixture(); const first = await releaseFixture(f.root, "1.0.0-rc.1"); const second = await releaseFixture(f.root, "1.0.0-rc.2"); const source = join(f.root, "private-rules.md"); await writeFile(source, "preserve\n"); await new InstallerTransaction({ ...f.request, releaseRoot: first, localRules: source }).install(); const result = await new InstallerTransaction({ ...f.request, releaseRoot: second, dryRun: true }).update(); const localRules = result.plan?.resources.find((resource) => resource.id === "local-rules"); assert.equal(localRules?.target, join(f.installationRoot, "releases", "1.0.0-rc.2", "bundle", "agent-governance", "local", "user-rules.md")); assert.equal(localRules.operation, "replace"); await assert.rejects(access(join(f.installationRoot, "releases", "1.0.0-rc.2"))); });
