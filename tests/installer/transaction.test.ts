...
test("rollback rejects an installed release under a mismatched version directory before mutation", async () => { const f = await fixture(); await new InstallerTransaction(f.request).install(); const other = await releaseFixture(f.root, "1.0.0-rc.2"); const installed = join(f.installationRoot, "releases", "1.0.0-rc.1"); rmSync(join(installed, "bundle"), { recursive: true, force: true }); cpSync(join(other, "bundle"), join(installed, "bundle"), { recursive: true }); writeFileSync(join(installed, "VERSION"), readFileSync(join(other, "VERSION"))); writeFileSync(join(installed, "release.files.sha256"), readFileSync(join(other, "release.files.sha256"))); const bindingRoot = await bindingStateRoot(f.installationRoot); const receiptPath = join(bindingRoot, "last-transaction.json"); const receipt = JSON.parse(await readFile(receiptPath, "utf8")) as Record<string, unknown>; await writeFile(join(String(receipt.backupRoot), "receipt.json"), `${JSON.stringify({ ...receipt, status: "PREPARED" })}\n`); let reachedEntryMutation = false; const rollback = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint === "beforeRollbackEntry") reachedEntryMutation = true; } }); assert.equal((await rollback.status()).state, "RECOVERY_REQUIRED"); await assert.rejects(rollback.rollback(), /installed release version does not match its directory/); assert.equal(reachedEntryMutation, false); await access(installed); });
...
test("update dry-run reports creation of local rules carried into a new release", async () => { const f = await fixture(); const first = await releaseFixture(f.root, "1.0.0-rc.1"); const second = await releaseFixture(f.root, "1.0.0-rc.2"); const source = join(f.root, "private-rules.md"); await writeFile(source, "preserve\n"); await new InstallerTransaction({ ...f.request, releaseRoot: first, localRules: source }).install(); const result = await new InstallerTransaction({ ...f.request, releaseRoot: second, dryRun: true }).update(); const localRules = result.plan?.resources.find((resource) => resource.id === "local-rules"); assert.equal(localRules?.target, join(f.installationRoot, "releases", "1.0.0-rc.2", "bundle", "agent-governance", "local", "user-rules.md")); assert.equal(localRules.operation, "create"); await assert.rejects(access(join(f.installationRoot, "releases", "1.0.0-rc.2"))); });
test("fresh-install dry-run reports replacement of existing shared local rules", async () => { const f = await fixture(); const source = join(f.root, "initial.md"); await writeFile(source, "initial\n"); await new InstallerTransaction({ ...f.request, localRules: source }).install(); const targetRoot = join(f.root, "second-target"); await mkdir(targetRoot); const replacement = join(f.root, "replacement.md"); await writeFile(replacement, "replacement\n"); const result = await new InstallerTransaction({ ...f.request, targetRoot, localRules: replacement, dryRun: true }).install(); const localRules = result.plan?.resources.find((resource) => resource.id === "local-rules"); assert.equal(localRules?.operation, "replace"); });
...
test("release staging does not write into a substituted stage directory", async () => { const f = await fixture(); let replacement = ""; const marker = Buffer.from("foreign stage marker\n"); const tx = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterReleaseStageCreation") return; const backups = join(f.installationRoot, "backups"); const binding = readdirSync(backups)[0]!; const transaction = readdirSync(join(backups, binding))[0]!; const stage = join(backups, binding, transaction, "release.stage"); renameSync(stage, `${stage}-retired`); mkdirSync(stage); writeFileSync(join(stage, "marker.bin"), marker); replacement = stage; } }); await assert.rejects(tx.install()); assert.deepEqual(await readFile(join(replacement, "marker.bin")), marker); await assert.rejects(access(join(replacement, "bundle"))); await assert.rejects(access(f.entry)); });
test("forward activation rejects a replacement binding root before creating current metadata", async () => { const f = await fixture(); const marker = Buffer.from("foreign binding marker\n"); let replacement = ""; const tx = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterReceipt") return; const binding = join(f.installationRoot, "bindings", readdirSync(join(f.installationRoot, "bindings"))[0]!); renameSync(binding, `${binding}-retired`); mkdirSync(binding); writeFileSync(join(binding, "marker.bin"), marker); replacement = binding; } }); await assert.rejects(tx.install(), /binding root identity changed|rollback failed/); assert.deepEqual(await readFile(join(replacement, "marker.bin")), marker); await assert.rejects(access(join(replacement, "current.json"))); });
test("forward activation rejects a copied replacement release stage before moving it", async () => { const f = await fixture(); let stage = ""; let retired = ""; const tx = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterReceipt") return; const binding = join(f.installationRoot, "bindings", readdirSync(join(f.installationRoot, "bindings"))[0]!); const receipt = JSON.parse(readFileSync(join(binding, "last-transaction.json"), "utf8")) as { backupRoot: string }; stage = join(receipt.backupRoot, "release.stage"); retired = `${stage}-retired`; renameSync(stage, retired); cpSync(retired, stage, { recursive: true }); } }); await assert.rejects(tx.install(), /identity (changed|validation)|rollback failed/); assert.equal(statSync(stage).isDirectory(), true); assert.equal(statSync(retired).isDirectory(), true); await assert.rejects(access(join(f.installationRoot, "releases", "1.0.0-rc.1"))); });
test("forward activation rejects a replacement staged local-rules parent before writing", async () => { const f = await fixture(); const rules = join(f.root, "rules.md"); const marker = Buffer.from("foreign local parent marker\n"); await writeFile(rules, "private rule\n"); let localParent = ""; const tx = new InstallerTransaction({ ...f.request, localRules: rules, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterReceipt") return; const binding = join(f.installationRoot, "bindings", readdirSync(join(f.installationRoot, "bindings"))[0]!); const receipt = JSON.parse(readFileSync(join(binding, "last-transaction.json"), "utf8")) as { backupRoot: string }; localParent = join(receipt.backupRoot, "release.stage", "bundle", "agent-governance", "local"); renameSync(localParent, `${localParent}-retired`); mkdirSync(localParent); writeFileSync(join(localParent, "marker.bin"), marker); } }); await assert.rejects(tx.install(), /local rules parent identity changed|rollback failed/); assert.deepEqual(await readFile(join(localParent, "marker.bin")), marker); await assert.rejects(access(join(localParent, "user-rules.md"))); });
...
test("rollback resumes a deterministic same-filesystem entry detach after interruption", async () => {
  ...
  assert.equal((await new InstallerTransaction(f.request).rollback()).state, "FRESH"); assert.deepEqual(await readFile(f.entry), original); assert.deepEqual(await readFile(join(dirname(f.entry), detached!)), applied);
});
...
test("rollback never overwrites a detach-path collision", async () => {
  ...
  const colliding = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "beforeRollbackEntryDetach") return; const receipt = JSON.parse(readFileSync(join(f.installationRoot, "bindings", readdirSync(join(f.installationRoot, "bindings"))[0]!, "last-transaction.json"), "utf8")) as { id: string }; detached = `.${basename(f.entry)}.agent-governance-${receipt.id}.restore.bin`; writeFileSync(join(dirname(f.entry), detached), foreign); } });
  await assert.rejects(colliding.rollback()); assert.deepEqual(await readFile(join(dirname(f.entry), detached!)), foreign);
});

test("rollback never binds a substituted entry parent before native detach", async () => {
  const f = await fixture(); await writeFile(f.entry, "original\n"); await new InstallerTransaction(f.request).install(); const installed = await readFile(f.entry); const retired = `${f.targetRoot}-retired`; const foreign = Buffer.from("foreign entry\n");
  const substituted = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "beforeRollbackEntryDetachMove") return; renameSync(f.targetRoot, retired); mkdirSync(f.targetRoot); writeFileSync(f.entry, foreign); } });
  await assert.rejects(substituted.rollback(), /identity changed/); assert.deepEqual(await readFile(f.entry), foreign); assert.deepEqual(await readFile(join(retired, basename(f.entry))), installed); assert.equal(readdirSync(f.targetRoot).some((name) => name.endsWith(".restore.bin")), false);
});

test("rollback keeps foreign replacement bytes untouched after native handles are bound", async () => {
  const f = await fixture(); await writeFile(f.entry, "original\n"); await new InstallerTransaction(f.request).install(); const installed = await readFile(f.entry); const retired = `${f.targetRoot}-retired`; const foreign = Buffer.from("foreign entry\n");
  const substituted = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterRollbackNativeDirectoriesBound") return; renameSync(f.targetRoot, retired); mkdirSync(f.targetRoot); writeFileSync(f.entry, foreign); } });
  await assert.rejects(substituted.rollback()); assert.deepEqual(await readFile(f.entry), foreign); const detached = readdirSync(retired).find((name) => name.endsWith(".restore.bin")); assert.notEqual(detached, undefined); assert.deepEqual(await readFile(join(retired, detached!)), installed);
});

test("recovery never trusts a replacement parent containing copied detach evidence", async () => {
  const f = await fixture(); await writeFile(f.entry, "original\n"); await new InstallerTransaction(f.request).install(); const retired = `${f.targetRoot}-retired`; const marker = Buffer.from("foreign marker\n");
  const interrupted = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterRollbackNativeDirectoriesBound") return; renameSync(f.targetRoot, retired); mkdirSync(f.targetRoot); writeFileSync(f.entry, "foreign entry\n"); writeFileSync(join(f.targetRoot, "marker.bin"), marker); } });
  await assert.rejects(interrupted.rollback()); const detached = readdirSync(retired).find((name) => name.endsWith(".restore.bin")); assert.notEqual(detached, undefined); const evidence = await readFile(join(retired, detached!)); await rm(f.entry); await writeFile(join(f.targetRoot, detached!), evidence);
  await assert.rejects(new InstallerTransaction(f.request).rollback(), /identity|parent|stale/i); await assert.rejects(access(f.entry)); assert.deepEqual(await readFile(join(f.targetRoot, "marker.bin")), marker); assert.deepEqual(await readFile(join(retired, detached!)), evidence);
});

test("rollback rejects a replacement binding root containing copied current state", async () => {
  const f = await fixture(); await writeFile(f.entry, "original\n"); await new InstallerTransaction(f.request).install(); const binding = await bindingStateRoot(f.installationRoot); const retired = `${binding}-retired`; const applied = await readFile(join(binding, "current.json")); const marker = Buffer.from("foreign binding marker\n");
  const rollback = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterRollbackEntry") return; renameSync(binding, retired); mkdirSync(binding); writeFileSync(join(binding, "current.json"), applied); writeFileSync(join(binding, "marker.bin"), marker); } });
  await assert.rejects(rollback.rollback(), /binding|identity|stale/i); assert.deepEqual(await readFile(join(binding, "current.json")), applied); assert.deepEqual(await readFile(join(binding, "marker.bin")), marker);
});

test("rollback rejects a replacement local-rules parent containing copied applied state", async () => {
  const f = await fixture(); const first = join(f.root, "first.md"); const second = join(f.root, "second.md"); await writeFile(first, "first\n"); await writeFile(second, "second\n"); await new InstallerTransaction({ ...f.request, localRules: first }).install(); await new InstallerTransaction({ ...f.request, localRules: second }).update(); const rules = join(f.installationRoot, "releases", "1.0.0-rc.1", "bundle", "agent-governance", "local"); const retired = `${rules}-retired`; const marker = Buffer.from("foreign rules marker\n");
  let replaced = false; const rollback = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterRollbackCurrent") return; replaced = true; renameSync(rules, retired); mkdirSync(rules); writeFileSync(join(rules, "user-rules.md"), "second\n"); writeFileSync(join(rules, "marker.bin"), marker); } });
  await assert.rejects(rollback.rollback()); assert.equal(replaced, true); assert.equal(await readFile(join(rules, "user-rules.md"), "utf8"), "second\n"); assert.deepEqual(await readFile(join(rules, "marker.bin")), marker);
});

test("rollback rejects an identically copied replacement release before cleanup", async () => {
  const f = await fixture(); await new InstallerTransaction(f.request).install(); const release = join(f.installationRoot, "releases", "1.0.0-rc.1"); const retired = `${release}-retired`;
  const rollback = new InstallerTransaction({ ...f.request, onCheckpoint: (checkpoint) => { if (checkpoint !== "afterRollbackCurrent") return; renameSync(release, retired); cpSync(retired, release, { recursive: true }); } });
  await assert.rejects(rollback.rollback(), /release|identity|stale/i); await access(join(release, "VERSION")); await access(join(retired, "VERSION"));
});

test("rollback never follows a substituted detach path during finalization", async () => {
  ...
  await assert.rejects(substituted.rollback(), /identity changed|symlink/); assert.notEqual(moved, undefined); await access(join(dirname(f.entry), moved!));
});
