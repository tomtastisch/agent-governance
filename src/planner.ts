...
export function planInstallation(input: { command: InstallerCommand; state: InstallState; entryPath: string; installationRoot: string; version: string; localRules: boolean; localRulesPath?: string; bindingId?: string; }): InstallPlan {
  ...
  if (input.localRules) {
    ...
    resources.push({ id: "local-rules", target: join(input.installationRoot, "releases", input.version, "bundle", "agent-governance", input.localRulesPath), operation: input.command === "update" ? "replace" : activeOperation });
  }
