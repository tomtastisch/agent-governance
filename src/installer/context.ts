...
import { optionalBytes, requiredBytes, requiredSnapshot } from "./snapshot.ts";
...
export interface Context { readonly target: TargetInspection & { readonly entrySnapshot?: FileSnapshotIdentity }; readonly release: VerifiedRelease; readonly binding: GovernanceBinding; readonly current?: CurrentMetadata; readonly installedLocalRules?: Buffer; readonly entry: Buffer; readonly state: InstallResult["state"]; }
...
export async function resolveContext(request: TransactionRequest, bindingId: string, currentPath: string, receiptPath: string): Promise<Context> {
  const inspected = await inspectTarget(request.targetRoot, request.entryFile, request.installationRoot);
  const snapshot = inspected.entryExists ? await requiredSnapshot(inspected.entryPath, "inspected entry") : undefined;
  if (snapshot !== undefined && (snapshot.identity.device !== inspected.entryIdentity?.device || snapshot.identity.inode !== inspected.entryIdentity.inode || snapshot.identity.mode !== inspected.entryIdentity.mode)) throw new ConcurrentEntryChange("entry identity changed during inspection");
  const target = { ...inspected, ...(snapshot === undefined ? {} : { entrySnapshot: snapshot.identity }) };
  const release = await verifyRelease(request.releaseRoot); const binding = buildBinding(release, request.installationRoot); const entry = snapshot?.bytes ?? Buffer.alloc(0); const currentBytes = await optionalBytes(currentPath); const receiptBytes = await optionalBytes(receiptPath);
