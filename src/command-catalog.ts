...
function validateCommand(raw: unknown): PublicCommandDefinition {
  ...
  if (typeof id !== "string" || !COMMAND_ID_PATTERN.test(id)) throw new Error("command id is invalid");
  ...
  if (typeof capability !== "string" || !CAPABILITIES.has(capability)) throw new Error(`command ${id} capability is invalid`);
  if (typeof effect !== "string" || !EFFECTS.has(effect)) throw new Error(`command ${id} effect is invalid`);
  ...
  return Object.freeze({
    id: id as PublicCommandId,
    path: Object.freeze([...(path as string[])]),
    description,
    capability: capability as PublicCommandDefinition["capability"],
    effect: effect as PublicCommandDefinition["effect"],
    orchestrates,
    interactive,
  });
}
